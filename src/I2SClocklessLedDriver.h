...
public:
  ...
  void initDMABuffers() {
    DMABuffersTampon[0] =
        allocateDMABuffer(nb_components * 8 * 2 * 3); // the buffers for the
    ...
    /*
     ...
     longh enough.
     */
    DMABuffersTransposed = (I2SClocklessLedDriverDMABuffer **)malloc(
        sizeof(I2SClocklessLedDriverDMABuffer *) * (num_led_per_strip + 2));
    for (int i = 0; i < num_led_per_strip + 2; i++) {
      if (i < num_led_per_strip + 1)
        ...
        DMABuffersTransposed[i] =
            allocateDMABuffer(nb_components * 8 * 2 * 3 * 4);
  ...
  void showPixelsFirstTranpose() { showPixelsFirstTranpose(NO_WAIT); }
    ...
    *)DMABuffersTransposed[j + 1]->buffer + 3 * 3 * 8);
    ...
    for (int j = 0; j < num_led_per_strip; j++) {
      loadAndTranspose(leds, num_led_per_strip, num_strips, _offsetDisplay,
                       DMABuffersTransposed[j + 1]->buffer, j, __green_map,
                       __red_map, __blue_map, __white_map, nb_components, p_g,
                       p_r, p_b);
    }
  }
  ...
  void setPixelinBuffer(uint32_t pos, uint8_t red, uint8_t green, uint8_t blue,
    ...
    // printf("nb c:%d\n",nb_components);
    uint8_t y = colors[0];
    *((uint16_t *)(B)) = (*((uint16_t *)(B)) & mask) |
                         ((uint16_t)((y & 128) >> 7) << stripNumber);
    *((uint16_t *)(B + 5)) = (*((uint16_t *)(B + 5)) & mask) |
                             ((uint16_t)((y & 64) >> 6) << stripNumber);
    *((uint16_t *)(B + 6)) = (*((uint16_t *)(B + 6)) & mask) |
                             ((uint16_t)((y & 32) >> 5) << stripNumber);
    ...
    *((uint16_t *)(B + 23)) =
        (*((uint16_t *)(B + 23)) & mask) | ((uint16_t)(y & 1) << stripNumber);
  ...
  void setPixelinBuffer(uint32_t pos, uint8_t red, uint8_t green,
                        uint8_t blue) {
    uint8_t W = 0;
    if (nb_components > 3) {
      W = MIN(red, green);
...
*/
  ...
  void showPixels() {
    ...
    for (int i = 0; i < 8; i++) {
      sprites[i].reorder(_offsetDisplay.panel_width,
                         _offsetDisplay.panel_height);
    }
#endif
    if (leds == NULL) {
    ...
    dmaBufferActive = 0;
    loadAndTranspose(leds, num_led_per_strip, num_strips, _offsetDisplay,
                     (uint16_t *)DMABuffersTampon[0]->buffer, ledToDisplay,
                     __green_map, __red_map, __blue_map, __white_map,
                     nb_components, p_g, p_r, p_b);
  ...
  I2SClocklessLedDriverDMABuffer *allocateDMABuffer(int bytes) {
    I2SClocklessLedDriverDMABuffer *b =
        (I2SClocklessLedDriverDMABuffer *)heap_caps_malloc(
            sizeof(I2SClocklessLedDriverDMABuffer), MALLOC_CAP_DMA);
    if (!b) {
...
static void IRAM_ATTR _I2SClocklessLedDriverinterruptHandler(void *arg) {
  ...
  if (GET_PERI_REG_BITS(I2S_INT_ST_REG(I2S_DEVICE), I2S_OUT_EOF_INT_ST_V,
    ...
    if (((I2SClocklessLedDriver *)arg)->transpose) {
      ...
      if (cont->ledToDisplay < cont->num_led_per_strip) {
        loadAndTranspose(
            cont->leds, cont->num_led_per_strip, cont->num_strips,
            cont->_offsetDisplay,
            (uint16_t *)cont->DMABuffersTampon[cont->dmaBufferActive]->buffer,
            cont->ledToDisplay, cont->__green_map, cont->__red_map,
            cont->__blue_map, cont->__white_map, cont->nb_components, cont->p_g,
            cont->p_r, cont->p_b);
        if (cont->ledToDisplay ==
            cont->num_led_per_strip -
                3) // here it's not -1 because it takes time top have the change
      ...
      if (cont->framesync) {
        ...
        xSemaphoreGiveFromISR(cont->I2SClocklessLedDriver_semSync,
                              &HPTaskAwoken);
  ...
  if (GET_PERI_REG_BITS(I2S_INT_ST_REG(I2S_DEVICE), I2S_OUT_TOTAL_EOF_INT_ST_V,
                        I2S_OUT_TOTAL_EOF_INT_ST_S)) {

    //        portBASE_TYPE HPTaskAwoken = 0;
    //            xSemaphoreGiveFromISR(((I2SClocklessLedDriver
    //            *)arg)->I2SClocklessLedDriver_semDisp, &HPTaskAwoken);
